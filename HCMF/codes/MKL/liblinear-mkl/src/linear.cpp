	...
	double Cp, double Cn, int solver_type)
{
	...
	for(i=0; i<l; i++)
	{
		...
		while (xi->index != -1)
		{
			QD[i] += (xi->value)*(xi->value)*prob->W[xi->index-1];
			xi++;
		}
		index[i] = i;
	}
	...
	while (iter < max_iter)
	{
		...
		for (s=0; s<active_size; s++)
		{
			...
			if(fabs(PG) > 1.0e-12)
			{
				...
				while (xi->index != -1)
				{
					w[xi->index-1] += prob->W[xi->index-1]*d*xi->value;
					xi++;
				}
			}
		}
...
model* train(const problem *prob, const parameter *param)
{
	...
	sub_prob.W = Malloc(double,sub_prob.n);
...
void cross_validation(const problem *prob, const parameter *param, int nr_fold, int *target)
{
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		subprob.W = Malloc(double,subprob.n);
		for(j=0;j<prob->n;j++)	
			subprob.W[j] = prob->W[j];
