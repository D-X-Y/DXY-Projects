...
{
	mexPrintf(
	"Usage: model = train(feature_weight_vector, training_label_vector, training_instance_matrix, 'liblinear_options', 'col');\n"
	"liblinear_options:\n"
	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			case 's':
				...
				if(param.solver_type != 3 && param.solver_type != 1)
				{
					mexPrintf("Solver type not supported\n");
					return 1;
				}
				break;
			case 'c':
