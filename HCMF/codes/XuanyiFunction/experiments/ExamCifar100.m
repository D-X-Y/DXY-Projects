...
log_dir = fullfile(pwd, 'results', 'cifar100-final');
mkdir_if_missing(log_dir);
%system(sprintf('rm -rf %s/*',log_dir));
log_file = fullfile(log_dir, ['Examples_Cifar100_ALM_', timestamp, '.txt']);
diary(log_file);
load ('Cifar100-12-CNN.mat');
...
save(fullfile(log_dir, 'cifar100.mat'), '-v7.3');
diary off;
