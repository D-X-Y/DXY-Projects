...
log_dir = fullfile(pwd, 'results', 'VGG_Animal');
mkdir_if_missing(log_dir);
%system(sprintf('rm -rf %s/*',log_dir));
log_file = fullfile(log_dir, ['Examples_VGG_Animal_', timestamp, '.txt']);
diary(log_file);
load ('VGG_animal.mat');
select = (1:8);
disp(select);
...
train_id     = randperm(total_number, ceil(total_number*ratio))';
