...
log_dir = fullfile(pwd, 'results', 'voc_t');
mkdir_if_missing(log_dir);
%system(sprintf('rm -rf %s/*',log_dir));
log_file = fullfile(log_dir, ['Examples_VOC_ALM_', timestamp, '.txt']);
diary(log_file);
load ('Caltech101.mat');
...
for index = 1:numel(features)
    ...
    model = train(train_label, sparse(train_data), '-s 2');
...
opts = default_alm();
opts.K = K;
opts.max_iters = 20;        opts.p = 1.01;        opts.mu = 1;
