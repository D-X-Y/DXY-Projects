function [image_roidb_train] = weakly_clean_data(classes, image_roidb_train, smallest)
  ...
  object_number_up = 4;
  for idx = 1:num
    %pseudo_boxes = weakly_nms_max_pseudo(image_roidb_train(idx).pseudo_boxes, 0.5);
    %if (isempty(pseudo_boxes)), continue; end
    pseudo_boxes = weakly_check_filter_img(image_roidb_train(idx).pseudo_boxes, smallest, object_number_up);
    if (isempty(pseudo_boxes)), continue; end
