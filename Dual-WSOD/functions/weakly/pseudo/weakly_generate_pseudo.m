function [image_roidb_train] = weakly_generate_pseudo(test_models, image_roidb_train)
    ...
    for inet = 1:num_nets
        ...
        for index = 1:num_roidb
            ...
            [~, cur_scores] = weakly_im_detect(conf, test_net, imread(image_roidb_train(index).image_path), image_roidb_train(index).boxes, conf.max_rois_num_in_gpu);
    ...
    for index = 1:num_roidb
        ...
        pseudo_boxes = generate_pseudo(final_boxes, final_score, num_classes, thresh_hold, image_roidb_train(index).image_label);
...
function pos_structs = generate_pseudo(final_boxes, final_score, num_classes, thresh_hold, image_label)
