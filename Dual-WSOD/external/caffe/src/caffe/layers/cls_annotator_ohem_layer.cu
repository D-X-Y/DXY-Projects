...
namespace caffe {  
  ...
  void ClsAnnotatorOHEMLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    num_imgs++;
    CHECK_GT(num_imgs, 0)
      << "number of images must be greater than 0 at ClsAnnotatorOHEMLayer";
  ...
  template <typename Dtype>
  void ClsAnnotatorOHEMLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
    return;
  }

  INSTANTIATE_LAYER_GPU_FUNCS(ClsAnnotatorOHEMLayer);
