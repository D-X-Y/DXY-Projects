...
local function createModel(opt)
   ...
   local function acc_until(nIn, nOut, kernel, stride, pad, name)
      ...
      acc:add(Convolution(nIn, 1, kernel, kernel, stride, stride, pad, pad))
      acc:add(SBatchNorm(1))
      local temp  = ReLU(true)
      temp.name = name
      temp.in_channel = nIn
      temp.out_channel = nOut
      temp.kernel_s = kernel * kernel
      acc:add(temp)
   ...
   local function bottleneck_acc(nInputPlane, nOutputPlane, stride)
      ...
      if nInputPlane == nOutputPlane then -- most Residual Units have this shape      
         ...
         --convs:add(acc_until(nInputPlane,nBottleneckPlane,1,stride,0,'acc_1'))
        
         -- conv3x3
         convs:add(SBatchNorm(nBottleneckPlane))
         convs:add(ReLU(true))
         convs:add(acc_until(nBottleneckPlane,nBottleneckPlane,3,1,1,'acc_2'))
        
         -- conv1x1
         convs:add(SBatchNorm(nBottleneckPlane))
         convs:add(ReLU(true))
         convs:add(acc_until(nBottleneckPlane,nOutputPlane,1,1,0,'acc_3'))
   ...
   if opt.dataset == 'cifar10' then
      ...
      model:add(layer(bottleneck_acc, nStages[1], nStages[2], n, 1)) -- Stage 1 (spatial size: 32x32)
      model:add(layer(bottleneck_acc, nStages[2], nStages[3], n, 2)) -- Stage 2 (spatial size: 16x16)
      model:add(layer(bottleneck_acc, nStages[3], nStages[4], n, 2)) -- Stage 3 (spatial size: 8x8)
