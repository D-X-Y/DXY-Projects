...
local function createModel(opt)
   ...
   -- CIFAR networks
   local function basicblock_acc(n, stride, type)
      ...
      --s:add(Convolution(nInputPlane,n,3,3,stride,stride,1,1))
      s:add(acc_until(nInputPlane,n,3,stride,1,'acc_1'))
      s:add(SBatchNorm(n))
      s:add(ReLU(true))
      -- s:add(Convolution(n,n,3,3,1,1,1,1))
      s:add(acc_until(n,n,3,1,1,'acc_1'))
   ...
   elseif opt.dataset == 'cifar10' then
      ...
      model:add(layer(basicblock_acc, 16, n, 1))
      model:add(layer(basicblock_acc, 32, n, 2))
      model:add(layer(basicblock_acc, 64, n, 2))
      model:add(ShareGradInput(SBatchNorm(iChannels), 'last'))
      model:add(ReLU(true))
