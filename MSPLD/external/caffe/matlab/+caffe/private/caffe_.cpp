...
// Xuanyi Add For Solver
// Usage: caffe_('solver_set_max_iter', hSolver, iters)
static void solver_set_max_iter(MEX_ARGS) {
  mxCHECK(nrhs == 2 && mxIsStruct(prhs[0]) && mxIsDouble(prhs[1]),
      "Usage: caffe_('solver_set_max_iter', hSolver, iters)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  int max_iter = mxGetScalar(prhs[1]);
  solver->set_max_iter(max_iter);
}

// Usage: caffe_('solver_set_stepsize', hSolver, iters)
static void solver_set_stepsize(MEX_ARGS) {
  mxCHECK(nrhs == 2 && mxIsStruct(prhs[0]) && mxIsDouble(prhs[1]),
      "Usage: caffe_('solver_set_stepsize', hSolver, iters)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  int stepsize = mxGetScalar(prhs[1]);
  solver->set_stepsize(stepsize);
}
...
static handler_registry handlers[] = {
  ...
  { "solver_set_max_iter",           solver_set_max_iter            },
  { "solver_set_stepsize",           solver_set_stepsize            },
