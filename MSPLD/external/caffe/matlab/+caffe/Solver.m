classdef Solver < handle
  ...
  methods
    ...
    function set_max_iter(self, iters)
      CHECK(isscalar(iters) && iters > 0, 'iters must be positive integer');
      iters = double(iters);
      caffe_('solver_set_max_iter', self.hSolver_self, iters);
    end
    function set_stepsize(self, iters)
      CHECK(isscalar(iters) && iters > 0, 'iters must be positive integer');
      iters = double(iters);
      caffe_('solver_set_stepsize', self.hSolver_self, iters);
    end
