...
namespace caffe {
  ...
  /**
   * ClsAnnotatorOHEMLayer:
   *   Annotate box-cls labels for Online Hard Example Mining (OHEM) training
   */
  template <typename Dtype>
  class ClsAnnotatorOHEMLayer :public Layer<Dtype>{
  public:
    explicit ClsAnnotatorOHEMLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
    virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
    virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
    ...
    virtual inline const char* type() const { return "clsAnnotatorOHEM"; }
